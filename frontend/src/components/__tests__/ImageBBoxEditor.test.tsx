...
describe('ImageBBoxEditor', () => {
  ...
  it('in draw mode, pressing down on an existing box selects that box instead of starting a new one (so users can still drag/delete recognised boxes)', async () => {
    ...
    const committedBoxes = (calls.length ? calls[calls.length - 1]?.[1] : undefined) as
      | BoundingBox[]
      | undefined;
    if (committedBoxes) {
  ...
  it('commits keyboard nudges so batch review drafts can persist precise adjustments', async () => {
    const onBoxesChange = vi.fn();
    const onBoxesCommit = vi.fn();
    const boxes: BoundingBox[] = [
      {
    ...
    expect(onBoxesCommit).toHaveBeenCalled();
    const calls = onBoxesCommit.mock.calls;
    const nextBoxes = calls[calls.length - 1]?.[1] as BoundingBox[];
    expect(nextBoxes[0].x).toBeGreaterThan(boxes[0].x);
  });

  it('uses larger resize hit targets for touch review', async () => {
    const boxes: BoundingBox[] = [
      {
        id: 'target',
        x: 0.1,
    ...
    const handle = screen.getByRole('separator', { name: /Resize top-left|向左上调整大小/i });
    expect(handle).toHaveStyle({ width: '24px', height: '24px' });
  });

  it('deduplicates type config lookups for repeated box types', () => {
    const boxes: BoundingBox[] = [
      {
        id: 'first',
        x: 0.1,
    ...
    const getTypeConfig = vi.fn(() => ({ name: 'Custom', color: '#000000' }));
    ...
    expect(getTypeConfig).toHaveBeenCalledTimes(1);
  });
});
