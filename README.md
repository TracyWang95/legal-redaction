...
| 模块 | 说明 |
|------|------|
| 🍏 **Mac 本地满血适配** | 原生支持 Apple Silicon (M系列)，支持无缝桥接本地 **Ollama** 引擎（无需再被弱小的 0.6B 模型束缚，自由运行 Qwen3:8B 等超级模型） |
| 📦 **一键批量处理** | 支持任意数量和格式的混合文档池，一键流水线自动脱敏，并支持 **ZIP 格式打包导出** |
| 🕒 **处理历史归档** | 自动留存本机的每一次脱敏任务痕迹，支持随时追溯、下载旧版本文件 |
| 📄 **多格式混排支持** | 旧版 Word (.doc)、新版 Word (.docx)、纯净 PDF与扫描件、图片 (.jpg/.png) |
| 🧠 **OCR + VLM 双引擎** | PaddleOCR-VL（文字识别）+ 视觉大语言模型（命名实体推演 / 印章指纹等视觉阻拦） |
| 👁️ **沉浸式标绘查验** | 前端支持超高清扫描件的**全自由面漫游拖拽**、无极缩放、以及精确到像素的框选脱敏 |
| 🔄 **脱敏模式** | 智能语义平替 / 掩码覆盖 (***) / 结构化标签替换 |
| 🧪 **本地自稳态优化** | 修复了 httpx 发起内网巡检时遭当地代理(Surge/Clash等)劫持造成的 502 问题，防断网运行 |
...
| **NER** | 本地 Ollama 模型 (如 qwen3:8b) | 命名实体识别 | 11434 |
...
- **操作系统**：macOS (Apple Silicon M系列深度适配) / Windows 10/11 / Linux
- **Python**：3.10+
- **Node.js**：18+
- **本地 LLM 引擎**：[Ollama](https://ollama.com/) (建议模型: `qwen3:8b`, `qwen2.5:7b`)
